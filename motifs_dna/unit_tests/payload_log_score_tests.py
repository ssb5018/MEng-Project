import pytest
import numpy as np
from ..motif_generation_tool.payload import payload_log_score as ls
...
async def test_hom_with_full_key_and_with_full_other_payloads():
    ...
    keyNum = 2
    payloadSize = 6
    constraints = get_constraints(keyNum=keyNum, maxHom=maxHom, keySize=keySize, payloadSize=payloadSize)
    hyperparams = get_hyperparameters(homHyperparam=homHyperparam)
    keys = ['AAAA', 'GGCC']
